...
def process_market_csv(file_path: Path):
    ...
    # 🔥 Save to logdata instead of data
    output_file = LOGDATA_DIR / (file_path.stem + "_log_returns.csv")
...
def main():
    for filename in FILES:
        file_path = DATA_DIR / filename   # 🔥 read from data/
        if not file_path.exists():
            raise FileNotFoundError(f"Missing file: {file_path}")
        process_market_csv(file_path)

    print("\nDone.")
    print("Use 'log_return' as the modeling column.")
